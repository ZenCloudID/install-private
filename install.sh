...
# Fungsi untuk mengelola pengguna administrator
admin_user_management() {
    while true; do
        ...
        echo " | | | | '_ \/ __| |/ _\` | __| | | |"
        echo " | |_| | | | \__ \ | (_| | |_| |_| |"
        echo "  \___/|_| |_|___/_|\__,_|\__|\__, |"
        echo "                              |___/ " 
        echo -e "${NC}"
        echo -e "${BLUE}===============================${NC}" | lolcat
        echo -e " Menu Pengguna Administrator"
        echo -e "${BLUE}===============================${NC}" | lolcat
        echo -e " 1. Buat Pengguna Administrator"
        echo -e " 2. Lihat Daftar Pengguna"
        echo -e " 3. Kembali ke Menu Utama"
        echo -e "${BLUE}===============================${NC}" | lolcat
        
        echo -ne " Pilih opsi [1-3]: "
        read -r choice

        case $choice in
            1) 
                echo -e "${GREEN}[+] Membuat pengguna administrator...${NC}" | lolcat
                cd "$THEME_DIR" && php artisan p:user:make
                echo -e "\n${GREEN}[✓] Proses pembuatan pengguna selesai${NC}" | lolcat
                ;;
            2)
                echo -e "${GREEN}[+] Daftar pengguna administrator:${NC}" | lolcat
                cd "$THEME_DIR" && php artisan p:user:list
                ;;
            3)
                break
                ;;
...
main_menu() {
    while true; do
        ...
        echo -e " 9. Admin User Management"
        echo -e "${BLUE}---------------------------------------------${NC}"
